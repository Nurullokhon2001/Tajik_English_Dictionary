...
class EngSearchAdapter(
    private val sportsList: List<EngSports>,
    private val listener: SportsAdapterListener
) : RecyclerView.Adapter<EngSearchAdapter.SearchViewHolder>() {
    ...
    override fun onBindViewHolder(holder: SearchViewHolder, position: Int) {
        val sports: EngSports = sportsList[position]

        var titlesubstrung : String? = sports.title?.substringAfter(")")

         //   kitty.substringBefore("@")) // murzik

        holder.titleTextView.text = titlesubstrung
    ...
    interface SportsAdapterListener {
        fun onSportSelected(sports: EngSports?)
    }
}